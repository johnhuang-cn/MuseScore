...
// needed?        _tabDur->setTrack(track());
    ...
    if (stt && stt->isJianpuStaff()) {
        ...
        String restStr(u"0");
        double w = FontMetrics::width(f, restStr) * mags;
        RectF bb = FontMetrics::boundingRect(f, restStr);

        // Bbox matches draw coordinates (text vertically centered around y=0)
        double halfH = bb.height() * mags / 2.0;
        double bboxTop = -halfH;

        // Reserve space for underlines: ascent + 8*mags covers up to 3 lines
        double ascent = -bb.y() * mags;
        double bboxBottom = ascent + 8.0 * mags;

        // bbox is extended for eighth-or-shorter rests to cover dirty-rect area of
        // duration underlines.
        double bboxWidth = w;
        if (item->durationType().hooks() > 0) {
            bboxWidth = w + spatium * 4.0;
        }
...
void RestLayout::alignRests(const System* system, LayoutContext& ctx)
{
    ...
    for (RestGroup& group : restGroups) {
        ...
        for (Rest* rest : group) {
            ...
            // Skip position override for Jianpu staff rests (they are fixed at y=0)
            if (!rest->staff() || !rest->staff()->staffTypeForElement(rest)->isJianpuStaff()) {
